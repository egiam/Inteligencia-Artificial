...
const getRandomProfile = (profileList) => {
    const randomIndex = Math.ceil(Math.random() * profileList.length - 1); //Devuelve un numero aleatoreo entre 0 y length - 1 (Ceil: Redondea hacia ariiba)
    const randomProfile = profileList[randomIndex]; // Agarra el jsonResponse[random]
...
const shuffleBtn = document.querySelector("button"); //Agarra el boton

shuffleBtn.addEventListener("click", editProfile); //Cuando se hace click en el boton se llama a la funcion de cambiar de profile